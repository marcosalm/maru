app.core.i18n = function() {
	...
	this.setLocale = function(newLocale) {
		...
		if (isLoaded) {
			app.core.log.warning('Localidade não pode ser alterado depois que os arquivos tenham sido carregados. Localidade mudança deve ocorrer no gancho "carregado".', 'Maru/Localization');
			return;
		}

		// Make sure locale is supported
		if (!app.config.localization.locales[newLocale]) {
			app.core.log.warning('Locale ' + newLocale + ' does not exist in app.config.localization.locales', 'Maru/Localization');
			return;
		}
	...
	this.get = function(key, properties) {
		...
		if (levels <= 1) {
			app.core.log.warning('Propriedade localização inválido[' + key + ']', 'Maru/Localization');
			return key;
		}
		
		// Prevent misunderstanding for deeply nested properties
		if (levels >= 5) {
			app.core.log.warning('Formato propriedade de localização é file.namespace.namespace.property, file.namespace.property ou file.property', 'Maru/Localization');
			return key;
		}
		...
		app.core.log.warning('Não foi localizado arquivo de localidade [' + key + ']', 'Maru/Localization');
		return key;
	};
	...
	var loadFiles = function(locale, callback) {
		app.core.log.debug('Started loading files for ' + locale, 'Maru/Localization');
		var files = app.config.localization.files;

		// Make sure there are files to load
		if (!files.length) {
			app.core.log.warning('Nenhum arquivo de localidade a ser carregado', 'Maru/Localization');
			return;
		}
				...
				loadFunctions.push(function(parallelCallback) {
					$.ajax({
						...
						success: function(source){ 
							app.core.log.debug('Carregado o arquivo de Localidade [' + locale + '/' + fileName + ']', 'Maru/Localization');
						...
						error: function(data) {
							app.core.log.warning('Falha ao carregar arquivo de Localidade [' + locale + '/' + fileName + ']', 'Maru/Localization');
							parallelCallback();
						}
					});
				});
